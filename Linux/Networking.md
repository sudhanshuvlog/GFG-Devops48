...
- IP address: a unique identifier assigned to each network interface. IPv4 addresses are 32-bit (example: 192.168.1.10). IPv6 addresses are 128-bit.
- Subnet mask / CIDR: defines which bits are network vs host (example: /24 = 255.255.255.0).


### Conditions for connectivity

- Both endpoints must have routable IP addresses (or appropriate NAT translations).
- There must be a path (physical and link layer) between them.
- If hosts are in the same subnet, they communicate directly; otherwise traffic is forwarded by routers.

## Tor and Onion Routing

- Provide unlinkability between sender and recipient by routing traffic over multiple hops.
- Use layered encryption so intermediate relays cannot see the full path or plaintext payload.

### Key components

- Directory authorities / consensus: a small set of trusted servers publish a signed network consensus that lists known relays, their IP addresses, public keys, flags (Guard, Exit, etc.), and descriptors. Clients (Tor software) download and cache this consensus.
- Relays (nodes): volunteer-run servers that advertise their IP, onion routing port, and public key in their descriptor.
- Client (Tor software): picks relays and builds circuits.

### How a Tor client selects a path

1. Client downloads the current consensus and fetches relay descriptors from directory caches. Relay descriptors contain each relay's IP address, ORPort (the port used to accept Tor connections), and the relay's long-term public key.
2. Client chooses a path of 3+ relays (common default: Guard, Middle, Exit) according to path selection rules (avoid same /16, respect flags, bandwidth weighting).

Important: the client already knows the IP addresses and public keys of candidate relays from the consensus/descriptors — it does not learn next-hop IPs from other relays during circuit construction.

### Circuit build and key agreement (how keys and next-hop info are provided)

1. The client opens a TCP connection to the first relay (a Guard). It performs a cryptographic handshake (Tor uses an authenticated key exchange) to negotiate a symmetric session key with that relay. This is done with cells called CREATE / CREATED during the circuit build.
2. To extend the circuit to the second relay, the client sends an EXTEND request encapsulated inside a RELAY cell over the existing circuit to the first relay. The EXTEND cell contains the IP:port of the next relay (from the consensus) and a handshake packet encrypted for that next relay using the next relay's public key.

How this works in practice:

- The client knows Relay B's IP and public key (from consensus). The client constructs an EXTEND cell whose payload includes Relay B's IP:port and a key-exchange message destined for Relay B.
- The first relay receives the RELAY cell, decrypts the outer layer (with its session key established with the client), and finds the RELAY command EXTEND plus the enclosed instruction: connect to Relay B at the given IP:port and forward the enclosed handshake bytes (which are encrypted for Relay B).
- The first relay then opens a TCP connection to Relay B (using the provided IP:port). The first relay sends the encapsulated handshake bytes to Relay B as part of the Tor protocol. Relay B responds to the handshake; that response is delivered back to the client through the first relay as a RELAY cell.

3. After the client receives the reply from Relay B (via Relay A), it completes the key exchange and now has a symmetric session key with Relay B. The client repeats EXTEND to create additional hops.

Key points about how a node learns the next hop IP

- The IP address of each relay is published in the network consensus and descriptors. The client includes the chosen next-hop IP:port inside the EXTEND instruction it sends (encrypted, inside a RELAY cell) to the previous hop.
- Intermediate relays are given the next hop IP:port explicitly by the client (inside the EXTEND message). The intermediate relay uses that information to open a TCP connection to the next relay.
- The intermediate relay does not learn the full path: it only learns the IP of the previous and next hop for that circuit and the session identifiers necessary to forward Tor cells.

Why the previous hop (Relay A) can forward to Relay B

- Relay A learns Relay B's IP because the client put Relay B's address inside the EXTEND payload (the client got that address earlier from the consensus). Relay A uses that IP to connect to Relay B and forward the handshake data. From Relay A's point of view it only knows how to contact Relay B — it does not know whether Relay B is the exit or the final destination.

Implementation-level notes (simplified)

- Cells: Tor uses small fixed-size cells. For circuit building, there are control cells CREATE/CREATED and relay cells with subcommands like EXTEND/EXTENDED and RELAY_DATA.
- Sessions: each client-relay pair has a symmetric key established during the CREATE/CREATED handshake. RELAY cells are encrypted with the session key for the intended hop; only the hop with the correct key can decrypt its layer.
- Forwarding: After decrypting the RELAY cell's outer layer, a relay finds either payload to forward onwards or commands (like EXTEND) containing the next-hop address and encrypted handshake.

Hidden services / rendezvous (brief)

- For hidden services (onion services), clients and services use introduction and rendezvous points — both discovered via the consensus or service descriptors — to build circuits to a rendezvous node without revealing identities to each other. The same mechanism (client/instructor providing next-hop info) is used to extend circuits.

Security and privacy caveats

- Directory consensus integrity is critical: clients rely on signed consensus to know relays' IPs and keys.
- A malicious relay can see which IP it is connecting to next (because the client instructs it), but it cannot see the entire circuit or the origin/destination beyond its neighbors.

### Short sequence summary (practical)

1. Client downloads consensus → learns relay A (IP, key), relay B (IP, key), relay C (IP, key).
2. Client opens TCP to A and performs key exchange (CREATE).
3. Client sends RELAY(EXTEND, payload: IP:port of B + handshake bytes encrypted for B) over the circuit to A.
4. A opens TCP to B using IP:port and forwards handshake bytes to B.
5. B responds; the response is returned to the client through A. Client completes key negotiation with B.
6. Repeat to add C.

Result: each relay only learns the immediate neighbors' IPs; the client is the only entity that knows the entire path.
